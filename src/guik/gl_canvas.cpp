...
namespace guik
{
   ...
   * @param size
   */
  GLCanvas::GLCanvas(const std::string &data_directory, const Eigen::Vector2i &size) : size(size), point_size(0.0f), keyframe_scale(1.0f), min_z(-20.0f), max_z(20.0f), z_clipping(true)
  {
  ...
  Eigen::Vector4i GLCanvas::pick_info(const Eigen::Vector2i &p, int window) const
  {
    ...
    std::sort(ps.begin(), ps.end(), [=](const Eigen::Vector2i &lhs, const Eigen::Vector2i &rhs)
