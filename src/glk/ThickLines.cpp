...
bool ThickLinesRenderer::initialize()
{
    if (!createShader())
    {
...
void ThickLinesRenderer::drawCoordinateFrames(const Eigen::Matrix4f &view,
                                              ...
                                              float line_thickness)
{
    ...
    // ✅ REMOVE DEBUG: This prints every frame and might cause performance issues
    ...
    //     std::cout << "🔍 TF Frames changed: " << last_frame_count << " → " << total_frames << std::endl;
    ...
    // Set uniforms once
    glUniformMatrix4fv(view_matrix_loc_, 1, GL_FALSE, view.data());
    glUniformMatrix4fv(projection_matrix_loc_, 1, GL_FALSE, projection.data());
    glUniform1f(cylinder_radius_loc_, line_thickness * 0.01f);
    glUniform1f(cylinder_length_loc_, frame_size);

    // Draw fixed frame at origin
    if (!fixed_frame.empty())
    {
        Eigen::Matrix4f model = Eigen::Matrix4f::Identity();
        glUniformMatrix4fv(model_matrix_loc_, 1, GL_FALSE, model.data());
    ...
    {
        glUniformMatrix4fv(model_matrix_loc_, 1, GL_FALSE, transform.matrix().data());
        glDrawArrays(GL_POINTS, 0, 3);
    }

    // ❌ REMOVE THIS LINE - IT'S CAUSING THE MEMORY LEAK!
    // glFinish(); // This ensures GPU completes all operations before continuing
...
void ThickLinesRenderer::cleanup()
{
    if (vao_ != 0)
    {
...
bool ThickLinesRenderer::createShader()
{
...
void ThickLinesRenderer::cacheUniformLocations()
{
    view_matrix_loc_ = glGetUniformLocation(shader_program_, "view_matrix");
    projection_matrix_loc_ = glGetUniformLocation(shader_program_, "projection_matrix");
    model_matrix_loc_ = glGetUniformLocation(shader_program_, "model_matrix");
    cylinder_radius_loc_ = glGetUniformLocation(shader_program_, "cylinder_radius");
    cylinder_length_loc_ = glGetUniformLocation(shader_program_, "cylinder_length");
}

bool ThickLinesRenderer::checkShaderCompilation(GLuint shader, const std::string &type)
{
    GLint success;
...
void ThickLinesRenderer::setupBuffers()
{
