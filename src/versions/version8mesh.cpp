...
public:
    ...
    PlyMesh loadPlyBinaryLE(const std::string &path)
    {
        ...
        while (std::getline(in, line))
        {
            ...
            {
                std::istringstream ss(line);
                ss >> line >> line >> vertex_count;
            }
            else if (line.rfind("element face", 0) == 0)
            {
                std::istringstream ss(line);
                ss >> line >> line >> face_count;
            }
        ...
        // --- Read face data ---
        // According to the spec, each face begins with a 'uchar' count,
        // then that many 'int' indices :contentReference[oaicite:3]{index=3}.
        std::vector<uint32_t> indices;
        indices.reserve(face_count * 3); // assume triangles
        ...
        for (size_t i = 0; i < face_count; ++i)
        {
            ...
            {
                // If faces are quads or n-gons, you can fan-triangulate:
